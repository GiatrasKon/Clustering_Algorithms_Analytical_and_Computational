...
for i = 1:4
    ...
    X = [X; X_temp]; % Do not transpose, keep it as 400x2
    labels = [labels; i * ones(n_points(i), 1)]; % Create labels
end

% Plot the dataset
figure, hold on;
colors = ['r', 'g', 'b', 'm']; % Define the colors for each group
markers = ['.', '.', '.', '.']; % Define the markers for each group
for i = 1:4
...
% Clustering of the data with k-medians
...
% Run k-medians for the best clustering m=4
...
title(sprintf('Best K-medians Clustering Result for m = %d Clusters', m)); % Updated to say K-medians
xlabel('X Coordinate');
ylabel('Y Coordinate');
axis equal;
grid on;
